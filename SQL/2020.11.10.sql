...
select LPAD('Page 1',15,'*') from dual; -- 오른쪽에 page 1 생김, 전체 문자열 15자리, 나머지는 *
select RPAD('001212-3',14,'*') from dual;
select SUBSTR('ABCDEFG',3,4) from dual; -- 3번째부터 4개 자름
...
-- 숫자->문자
select to_char(10000.123, 'L000,000.00') from dual; -- 0 붙음
select to_char(10000, 'L999,999.99') from dual;     -- 0 안 붙음
...
-- 내가 몇일을 살았는지 확인
select trunc(sysdate - to_date('1993-09-17','YYYY-MM-DD')) from dual;
...
select  ename, deptno, 
    ...
    decode( deptno, 
        10, 'ACCOUNTING', -- DEPTNO=10이면 ACCOUNTING 출력
...
select 
    ...
    count(comm) as "커미션 수", -- null은 세지 않는다
    max(sal) as "가장 큰 급여액",
    min(sal) as "가장 적은 급여액"
from emp
;
...
-- 직무의 개수
select count(distinct job) as "직무의 개수"
from emp
order by job
;
