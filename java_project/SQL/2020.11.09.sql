...
-- nvl (컬럼, 대체값) : 컬럼의 값이 null 일 때 대체값으로 치환, 이 때 대체값은 컬럼의 타입에 맞는 데이터이어야 한다.
-- 연봉의 계산을 sal*12+comm으로 계산하자
...
select *
from emp
where hiredate='81/11/17'   -- 날짜 비교에서도 날짜는 작은 따옴표로 묶어서 사용
;
...
--다음은 부서번호가 10번이 아닌 사원의 사원이름, 부서번호, 직급을
--출력해 봅시다.
...
--2000에서 3000 사이의 급여를 받는 사원과 같이 특정 범위 내에 속하는
--데이터인지를 알아보기 위해서 비교연산자와 논리 연산자를 결합하여
--표현할 수 있습니다.
select *
from emp
...
select *
from emp
--where ename like 'F%' -- F로 시작하고 뒤에는 어떤 문자가 와도 상관없음
--where ename like '%S' -- 이름이 S로 끝나는 이름을 검색
--where ename like '%A%'-- 이름에 A문자를 포함하는 이름을 검색
--where ename like '_A%'-- 첫 번째 문자는 어떤 문자든 상관없고 두 번째 문자는 반드시 A 문자를 포함하는
--where ename like '__R%' -- 첫 번째, 두 번째 문자는 어떤 문자든 상관없고, 세 번째에는 R 문자를 포함하는 이름을 검색
-- 000000-1000000 '______1%'
-- '1987/12/19' -> '____12%' 991130 '__11__'
