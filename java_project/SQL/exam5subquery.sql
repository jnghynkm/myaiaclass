...
from emp a, (select deptno, min(sal) as ms from emp group by deptno) b
...
--54. 부서위치가 DALLAS인 사원의 
--이름과 부서번호 및 담당업무를 표시하시오.
...
select min(avgsal)
from (select job, avg(sal) as avgsal from emp group by job)
;
