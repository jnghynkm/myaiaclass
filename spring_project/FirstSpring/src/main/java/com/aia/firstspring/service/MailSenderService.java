...
//		SimpleMailMessage message = new SimpleMailMessage(templateMessage);
		message.setTo("jnghyn93@gmail.com");
	...
	public boolean mailSend(String toEmailAddress) {
		...
		try {
			...
			message.setFrom(new InternetAddress("jnghyn93@gmail.com")); 
			// 받는 사람의 이메일 주소 
			message.addRecipient(RecipientType.TO, new InternetAddress(toEmailAddress,"고고고 고객님","utf-8")); 
			// 메일 전송
			mailSender.send(message);
			
		} catch (MessagingException e) {
			System.out.println("인코딩 에러 발생1");
			result = false;
			e.printStackTrace();
